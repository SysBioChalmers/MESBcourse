...
The data needed for this exercise are the results from the previous differential expression analysis exercise, using the gene counts from glucose conditions (saved as **DE_results_glucose.txt** in the **data/DE_results/** subdirectory in the **RNAseq2_GSA** directory).

1. Start R and use setwd() (or Session > Set Working Directory > Choose Directory… from the top menu bar) to change the working directory to the RNAseq2_GSA/ directory, which should contain a subdirectory named data/.
```{r setup, include=FALSE,warning=FALSE,echo=TRUE,message=FALSE}
knitr::opts_chunk$set(echo = TRUE)
# setwd("~/Documents/Course/MESB/RNAseq2_GSA")
...
```{r GSA}
gsaRes <- runGSA(-log10(pvals)*sign(logfc), geneSetStat="gsea", gsc=gscGO, gsSizeLim=c(10, Inf), nPerm = 1000)
```
...
Another way to visualize these results instead of the network plot is using a heatmap. Piano’s GSAheatmap function is an easy way to display the results as a clustered heatmap. This heatmap draws one row per gene set, so it needs a tall plotting area; in a small plot panel R will otherwise throw `Error in plot.new() : figure margins too large`. The chunk below takes care of this for you: when the document is knitted it requests a large enough figure, and when run interactively it opens a separate, appropriately sized plotting window.

```{r GSAheatmap, fig.width=8, fig.height=10}
# GSAheatmap draws one row per gene set, so it needs a tall plotting region.
# When run interactively, open a separate window large enough to avoid the
# error "figure margins too large". When knitting, the chunk's fig.width /
# fig.height options already provide enough space, so the new window is skipped.
