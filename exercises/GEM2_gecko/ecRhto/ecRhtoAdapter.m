classdef ecRhtoAdapter < ModelAdapter
    methods
        function obj = ecRhtoAdapter()
            ...
            % Auto-detect the project folder as the location of this adapter
            % file (it lives in the ecRhto folder), so the exercise works
            % regardless of where the repository is cloned. If you prefer, you
            % can replace this with an explicit absolute path to your ecRhto
            % project folder.
            obj.params.path = fileparts(mfilename('fullpath'));
			% Path to the conventional GEM that this ecModel will be based on.
			obj.params.convGEM = fullfile(obj.params.path,'models','rhto.xml');

			% Average enzyme saturation factor
			obj.params.sigma = 0.5;

			% Total protein content in the cell [g protein/gDw]
			obj.params.Ptot = 0.4385;
			...
			obj.params.gR_exp = 0.18;

			% Provide your organism scientific name
			obj.params.org_name = 'Rhodotorula toruloides';
			...
			obj.params.uniprot.ID = 'UP000016926'; % should match the ID type
